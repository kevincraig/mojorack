...
export function DeviceGrid({ devices }: DeviceGridProps) {
  ...
  return (
    <div className="grid grid-cols-3 gap-1.5 overflow-y-auto max-h-full content-start">
      {devices.map((device) => (
        <div
          key={device.key}
          className="border border-[var(--line)] bg-black/30 px-1.5 py-1 flex flex-col gap-0.5 min-w-0"
        >
          <div className="flex items-baseline justify-between gap-1">
            <span className="text-[10px] font-bold uppercase truncate">{device.name}</span>
            <span className="text-[8px] text-[var(--text-dim)] uppercase shrink-0">{device.type}</span>
          </div>
          <div className="flex items-baseline justify-between text-[9px] gap-1">
          ...
          <div className="flex items-baseline justify-between text-[9px] text-[var(--text-dim)] gap-1">
